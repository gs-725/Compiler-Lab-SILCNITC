...
int getFLabel(char *name)
{
    ...
    exit(1); // Added exit for consistency
}
...
struct AST_Node *makeArrayLeafNode(char *name, struct AST_Node *l, char *s)
{
    ...
    if (new_node->GSTentry)
        ...
        // Handle local arrays (no GST entry)
        // Type is determined later by LST
    }
...
struct AST_Node *ASTChangeTypeGST(struct AST_Node *root, struct Typetable *type)
{
    if (root != NULL)
    {
        ...
        if (root->nodetype == VARIABLE || root->nodetype == ARRAY) // Include ARRAY for global array declarations
        {
    ...
    return NULL; // Corrected: function returns an AST_Node* which should be NULL upon deletion/end of recursion
}
...
struct AST_Node *ASTChangeTypeLST(struct LSTable *table, struct AST_Node *root, struct Typetable *type)
{
    if (root != NULL)
    {
        ...
        if (root->nodetype == VARIABLE) // Note: LST doesn't distinguish variable/array nodes, relies on lookup.
        {
...
int codeGen(struct AST_Node *t, struct LSTable *LST, FILE *target_file) // returns the register number
{
    ...
    else if (t->nodetype == FREE)
    {
        if (t->left->nodetype == VARIABLE)
            ...
            addrReg = getFieldAddr(t->left, LST, target_file);
        // Added Array access support for free()
        else if (t->left->nodetype == ARRAY)
            addrReg = getArrayAddr(t->left, LST, target_file); 
        else {
            printf("Error: Invalid argument to free()\n");
            exit(1);
        }
        
        p = getReg();
        fprintf(target_file, "MOV R%d, R%d\n", p, addrReg);
        fprintf(target_file, "MOV R%d, [R%d]\n", p, p); // Dereference to get the heap address
...
int getAddr(struct AST_Node *t, struct LSTable *LST, FILE *target_file)
{
    ...
    else
    {
        ...
        if (g == NULL) {
            printf("Error: Variable or Array \"%s\" not declared\n", name);
            exit(1);
        }
        int p = getReg();
        fprintf(target_file, "MOV R%d, %d\n", p, g->binding);
...
// MODIFIED: getArrayAddr now handles both Global and Local arrays.
int getArrayAddr(struct AST_Node *t, struct LSTable *LST, FILE *target_file)
{
    int p = getReg();
    
    // Determine base address of the array (which stores pointers)
    ...
    if (g != NULL) {
        ...
        // Local Array (using binding for offset from BP)
        fprintf(target_file, "MOV R%d, BP\n", p);
        fprintf(target_file, "ADD R%d, %d\n", p, l->binding);
    } else {
        printf("Error: Array \"%s\" not found (getArrayAddr)\n", t->name);
        exit(1);
    }
    ...
    fprintf(target_file, "ADD R%d, R%d\n", p, q);
    freeReg();
    return p; // Returns the address of the array element (which stores the heap pointer)
}


// In exprtree.c, around line 1324:
int getFieldAddr(struct AST_Node *t, struct LSTable *LST, FILE *target_file)
{
    ...
    if (t->nodetype == ARRAY) 
    {
        // Base Case 1: Array element (e.g., 'p[0]' in 'p[0].data')
        a = getArrayAddr(t, LST, target_file); // a holds &p[i]
        return a;
    }
    else if (t->nodetype == VARIABLE) 
    {
        // Base Case 2: Simple variable (e.g., 'p' in 'p.data' or 'h' in 'h.a')
        a = getAddr(t, LST, target_file); // a holds &p or &h
        return a;
    }
    else if (t->nodetype == FIELD) 
    {
        // Recursive step: 'a' holds the address of the parent pointer (e.g., &p, &p[i], or &h.left)
        a = getFieldAddr(t->left, LST, target_file); 
        
        struct FieldlistNode *f = TTLookupField(t->left->type, t->right->name);
        if (f == NULL) {
            printf("Error: Field '%s' not found in type '%s' during code generation.\n", t->right->name, t->left->type->name);
            exit(1);
        }
        int fieldIndex = f->fieldIndex;
        
        // 1. Dereference the parent's pointer: R_a now holds the heap address of the structure
        fprintf(target_file, "MOV R%d, [R%d]\n", a, a); 

        // 2. Add the field offset to get the final field's address
        fprintf(target_file, "ADD R%d, %d\n", a, fieldIndex);
        
        return a; // Return the address of the field (e.g., &p.data)
    }
...
struct LSTable *LSTParamInstall(struct LSTable *table, struct ParamNode *Phead)
{
    ...
    while (curr)
    {
        // LSTInstall returns LSTable*, not LST_Node*
        table = LSTInstall(table, curr->name, curr->type);
        // table->tail is the newly inserted node.
