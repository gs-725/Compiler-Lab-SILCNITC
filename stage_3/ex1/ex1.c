...
struct AST_Node *makeVarLeafNode(Type type, char varname, char *s)
{
...
struct AST_Node *makeConstLeafNode(Type type, int val, char *s)
{
    struct AST_Node *new_node = (struct AST_Node *)malloc(sizeof(struct AST_Node));
    new_node->s = (char *)malloc(sizeof(char) * strlen(s));
    new_node->s = strdup(s);
