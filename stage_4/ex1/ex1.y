...
var_list: var_list ',' ID_ {
                GSTInstall($3->varname, $3->type, 1, 0, 0);
                $$ = makeNode(STATEMENT, VOID, $1, NULL, $3, "VARLIST");
            }
        | ID_ {
                GSTInstall($1->varname, $1->type, 1, 0, 0);
                $$ = $1;
            }
		| var_list ',' ID_ '[' NUM_ ']' {
			GSTInstall($3->varname, $3->type, $5->val, 0, 1);
			$$ = makeNode(STATEMENT, VOID, $1, NULL, $3, "VARLIST");
		}
		| ID_ '[' NUM_ ']' {
			GSTInstall($1->varname, $1->type, $3->val, 0, 1);
			$$ = $1;
		}
		| var_list ',' ID_ '[' NUM_ ']' '[' NUM_ ']' {
			GSTInstall($3->varname, $3->type, $5->val, $8->val, 2);
			$$ = makeNode(STATEMENT, VOID, $1, NULL, $3, "VARLIST");
		}
		| ID_ '[' NUM_ ']' '[' NUM_ ']' {
			GSTInstall($1->varname, $1->type, $3->val, $6->val, 2);
			$$ = $1;
		};
...
id: ID_ {
    ...
    | ID_ '[' expr ']' '[' expr ']' {
		// Get the GST entry for the array name
		struct GST_Node *curr = GSTLookup($1->varname);
		if (curr == NULL) {
		    printf("Array \"%s\" not declared\n", $1->varname);
		    exit(1);
		...
		// Check if array indices are integers
		...
		// Create a new AST node for the 2D array access
