...
%token CLASS_ ENDCLASS_ EXTENDS_ NEW_
...
ClassDef        : Cname '{' DECL_ CFieldDeclList CMethodDecl ENDDECL_ CMethodDef '}' {
                    if (Cptr->fieldCount > 8) {
                        printf("Line %d: class \'%s\' can't have more than 8 fields\n", yylineno, Cptr->name);
                    }
                    if (Cptr->methodCount > 8) {
                        printf("Line %d: class \'%s\' can't have more than 8 methods\n", yylineno, Cptr->name);
                    }
                    Cptr = NULL;
                }
...
GDecl: Type GIdList ';' {
            if (CLookup($1->name) != NULL) {
                ASTChangeTypeClassGST($2, NULL, CLookup($1->name));
            } else {
                ASTChangeTypeClassGST($2, $1, NULL);
            }
        }
...
InputStmt: READ_ '(' id ')' { $$ = makeNode(READ, TTLookup("void"), NULL, $3, NULL, NULL, NULL, "READ");}
...
AsgnStmt: id '=' stringExpr {
        ...
        | id '=' NEW_ '(' ID_ ')' {
            struct ClassTable *class = CLookup($5->name);
            if (class == NULL) {
